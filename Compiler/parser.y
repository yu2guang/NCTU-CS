%{
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#include "header.h"
#include "symtab.h"
#include "semcheck.h"
#include "genCode.h"

extern int linenum;
extern FILE	*yyin;
extern char	*yytext;
extern char buf[256];
extern int Opt_Symbol;		/* declared in lex.l */

int scope = 0;
char fileName[256];
struct SymTable *symbolTable;
__BOOLEAN paramError;
struct PType *funcReturn;
__BOOLEAN semError = __FALSE;
int inloop = 0;

extern FILE* fout_code;
int isMain = 0;
int counterNum = 0;
int isConst = 0;
int genI2F = 0, genI2F2 = 0;
int ifLevelCur = -1, ifLevelStore = 0, ifLevelNext = 0;
int trueLevel = 0;
int tempLocal = 0;
SEMTYPE tempType;
int loopLevelCur = -1, loopLevelStore = 0, loopLevelNext = 0;
int localNo = 0;

%}

%union {
	int intVal;
	float floatVal;	
	char *lexeme;
	struct idNode_sem *id;
	struct ConstAttr *constVal;
	struct PType *ptype;
	struct param_sem *par;
	struct expr_sem *exprs;
	struct expr_sem_node *exprNode;
	struct constParam *constNode;
	struct varDeclParam* varDeclNode;
};

%token	LE_OP NE_OP GE_OP EQ_OP AND_OP OR_OP
%token	READ BOOLEAN WHILE DO IF ELSE TRUE FALSE FOR INT PRINT BOOL VOID FLOAT DOUBLE STRING CONTINUE BREAK RETURN CONST
%token	L_PAREN R_PAREN COMMA SEMICOLON ML_BRACE MR_BRACE L_BRACE R_BRACE ADD_OP SUB_OP MUL_OP DIV_OP MOD_OP ASSIGN_OP LT_OP GT_OP NOT_OP

%token <lexeme>ID
%token <intVal>INT_CONST 
%token <floatVal>FLOAT_CONST
%token <floatVal>SCIENTIFIC
%token <lexeme>STR_CONST

%type<ptype> scalar_type dim
%type<par> array_decl parameter_list
%type<constVal> literal_const
%type<constNode> const_list 
%type<exprs> variable_reference logical_expression logical_term logical_factor relation_expression arithmetic_expression term factor logical_expression_list literal_list initial_array
%type<intVal> relation_operator add_op mul_op dimension
%type<varDeclNode> identifier_list


%start program
%%

program : decl_list 
		  funct_def
		  decl_and_def_list 
		  {
		  	checkUndefinedFunc(symbolTable);
		  	if(Opt_Symbol == 1)
		  		printSymTable( symbolTable, scope );
		  }
		;

decl_list : decl_list var_decl
		  | decl_list const_decl
		  | decl_list funct_decl { counterNum = 0;}
		  |
		  ;


decl_and_def_list : decl_and_def_list var_decl
				  | decl_and_def_list const_decl
				  | decl_and_def_list funct_decl { counterNum = 0;}
				  | decl_and_def_list funct_def { counterNum = 0;}
				  | 
				  ;

		  
funct_def : scalar_type ID L_PAREN R_PAREN 
			{
				funcReturn = $1; 
				struct SymNode *node;
				node = findFuncDeclaration( symbolTable, $2 );
				
				if( node != 0 ){
					verifyFuncDeclaration( symbolTable, 0, $1, node );
				}
				else{
					insertFuncIntoSymTable( symbolTable, $2, 0, $1, scope, __TRUE );
				}

				/* gen funct start */
				genFunctStart($1->type,$2,NULL,&isMain);
			}
			compound_statement {
				/* gen funct end */
				if(isMain==1){
					genFunctEnd(-1);
					isMain = 0;
				} 
				genFunctEnd(-2);

				funcReturn = 0; 
			}	
		  | scalar_type ID L_PAREN parameter_list R_PAREN  
			{
				funcReturn = $1;
				
				paramError = checkFuncParam( $4 );
				if( paramError == __TRUE ){
					fprintf( stdout, "########## Error at Line#%d: param(s) with several fault!! ##########\n", linenum );
					semError = __TRUE;
				}
				// check and insert function into symbol table
				else{
					struct SymNode *node;
					node = findFuncDeclaration( symbolTable, $2 );

					if( node != 0 ){
						if(verifyFuncDeclaration( symbolTable, $4, $1, node ) == __TRUE){	
							insertParamIntoSymTable( symbolTable, $4, scope+1 );
						}				
					}
					else{
						insertParamIntoSymTable( symbolTable, $4, scope+1 );				
						insertFuncIntoSymTable( symbolTable, $2, $4, $1, scope, __TRUE );
					}

					/* gen funct start */
					genFunctStart($1->type,$2,$4,&isMain);
				}
			} 	
			compound_statement { 
				/* gen funct end */
				if(isMain==1){
					genFunctEnd(-1);
					isMain = 0;
				}
				genFunctEnd(-2);

				funcReturn = 0; 
			}
		  | VOID ID L_PAREN R_PAREN 
			{
				funcReturn = createPType(VOID_t); 
				struct SymNode *node;
				node = findFuncDeclaration( symbolTable, $2 );

				if( node != 0 ){
					verifyFuncDeclaration( symbolTable, 0, createPType( VOID_t ), node );					
				}
				else{
					insertFuncIntoSymTable( symbolTable, $2, 0, createPType( VOID_t ), scope, __TRUE );	
				}

				/* gen funct start */
				genFunctStart(VOID_t,$2,NULL,&isMain);
			}
			compound_statement { 
				/* gen funct end */
				if(isMain==1){
					isMain = 0;
				}
				genFunctEnd(-1);
				genFunctEnd(-2);

				funcReturn = 0; 
			}	
		  | VOID ID L_PAREN parameter_list R_PAREN
			{	
				funcReturn = createPType(VOID_t);
				
				paramError = checkFuncParam( $4 );
				if( paramError == __TRUE ){
					fprintf( stdout, "########## Error at Line#%d: param(s) with several fault!! ##########\n", linenum );
					semError = __TRUE;
				}
				// check and insert function into symbol table
				else{
					struct SymNode *node;
					node = findFuncDeclaration( symbolTable, $2 );

					if( node != 0 ){
						if(verifyFuncDeclaration( symbolTable, $4, createPType( VOID_t ), node ) == __TRUE){	
							insertParamIntoSymTable( symbolTable, $4, scope+1 );				
						}
					}
					else{
						insertParamIntoSymTable( symbolTable, $4, scope+1 );				
						insertFuncIntoSymTable( symbolTable, $2, $4, createPType( VOID_t ), scope, __TRUE );
					}
				}

				/* gen funct start */
				genFunctStart(VOID_t,$2,$4,&isMain);
			} 
			compound_statement { 
				/* gen funct end */
				if(isMain==1){
					isMain = 0;
				}
				genFunctEnd(-1);
				genFunctEnd(-2);

				funcReturn = 0; 
			}		  
		  ;

funct_decl : scalar_type ID L_PAREN R_PAREN SEMICOLON
			{
				insertFuncIntoSymTable( symbolTable, $2, 0, $1, scope, __FALSE );	
			}
		   | scalar_type ID L_PAREN parameter_list R_PAREN SEMICOLON
		    {
				paramError = checkFuncParam( $4 );
				if( paramError == __TRUE ){
					fprintf( stdout, "########## Error at Line#%d: param(s) with several fault!! ##########\n", linenum );
					semError = __TRUE;
				}
				else {

					struct param_sem *tempParam = $4;
					insertFuncIntoSymTable( symbolTable, $2, $4, $1, scope, __FALSE );
				}
			}
		   | VOID ID L_PAREN R_PAREN SEMICOLON
			{				
				insertFuncIntoSymTable( symbolTable, $2, 0, createPType( VOID_t ), scope, __FALSE );
			}
		   | VOID ID L_PAREN parameter_list R_PAREN SEMICOLON
			{
				paramError = checkFuncParam( $4 );
				if( paramError == __TRUE ){
					fprintf( stdout, "########## Error at Line#%d: param(s) with several fault!! ##########\n", linenum );
					semError = __TRUE;	
				}
				else {
					insertFuncIntoSymTable( symbolTable, $2, $4, createPType( VOID_t ), scope, __FALSE );
				}
			}
		   ;

parameter_list : parameter_list COMMA scalar_type ID
			   {
				struct param_sem *ptr;
				ptr = createParam( createIdList( $4 ), $3 );

				/* gen var */
				//local
				ptr->isLocal = 1;
				ptr->stackNum = counterNum;
				counterNum++;

				param_sem_addParam( $1, ptr );
				$$ = $1;
			   }
			   | parameter_list COMMA scalar_type array_decl
			   {
				$4->pType->type= $3->type;
				param_sem_addParam( $1, $4 );
				$$ = $1;
			   }
			   | scalar_type array_decl 
			   { 
				$2->pType->type = $1->type;  
				$$ = $2;
			   }
			   | scalar_type ID { 
					struct param_sem *ptr;

			   		ptr = createParam( createIdList( $2 ), $1 ); 

			   		/* gen var */
					//local
					ptr->isLocal = 1;
					ptr->stackNum = counterNum;
					counterNum++;

					$$ = ptr;
			   	}
			   ;

var_decl : scalar_type identifier_list SEMICOLON
			{
				struct varDeclParam *ptr;
				struct SymNode *newNode;
				for( ptr=$2 ; ptr!=0 ; ptr=(ptr->next) ) {						
					if( verifyRedeclaration( symbolTable, ptr->para->idlist->value, scope ) == __FALSE ) { }
					else {
						if( verifyVarInitValue( $1, ptr, symbolTable, scope ) ==  __TRUE ){	
							newNode = createVarNode( ptr->para->idlist->value, scope, ptr->para->pType );

							/* gen var */
							if(scope==0){ //global
								newNode->isLocal = 0;
								//genAssignValue("global",0,ptr->para->idlist->value,ptr->para->pType->type);
							}
							else{ //local
								newNode->isLocal = 1;
								newNode->stackNum = counterNum;
								counterNum++;
							}

							insertTab( symbolTable, newNode );																		
						}
					}
				}
			}
			;

identifier_list : identifier_list COMMA ID
				{					
					struct param_sem *ptr;	
					struct varDeclParam *vptr;				
					ptr = createParam( createIdList( $3 ), createPType( VOID_t ) );
					vptr = createVarDeclParam( ptr, 0 );	
					addVarDeclParam( $1, vptr );
					$$ = $1; 

					if(scope==0){ // global
                		genGlobalVar(tempType,$3);
                	}	
                	tempLocal++;
				}
                | identifier_list COMMA ID 
                {
                	if(scope==0){ // global
                		genGlobalVar(tempType,$3);
                	}
                }
                  ASSIGN_OP logical_expression              
				{
					struct param_sem *ptr;	
					struct varDeclParam *vptr;				
					ptr = createParam( createIdList( $3 ), createPType( VOID_t ) );
					vptr = createVarDeclParam( ptr, $6 );
					vptr->isArray = __TRUE;
					vptr->isInit = __TRUE;	
					addVarDeclParam( $1, vptr );	
					$$ = $1;
					
					if(scope!=0){ //local
                		genAssignValue("local",tempLocal,NULL,tempType);
						genI2F2 = 0;
                	}	
					tempLocal++;
				}
                | identifier_list COMMA array_decl ASSIGN_OP initial_array
				{
					struct varDeclParam *ptr;
					ptr = createVarDeclParam( $3, $5 );
					ptr->isArray = __TRUE;
					ptr->isInit = __TRUE;
					addVarDeclParam( $1, ptr );
					$$ = $1;	
				}
                | identifier_list COMMA array_decl
				{
					struct varDeclParam *ptr;
					ptr = createVarDeclParam( $3, 0 );
					ptr->isArray = __TRUE;
					addVarDeclParam( $1, ptr );
					$$ = $1;
				}
                | array_decl ASSIGN_OP initial_array
				{	
					$$ = createVarDeclParam( $1 , $3 );
					$$->isArray = __TRUE;
					$$->isInit = __TRUE;	
				}
                | array_decl 
				{ 
					$$ = createVarDeclParam( $1 , 0 ); 
					$$->isArray = __TRUE;
				}
                | ID 
                {
                	if(scope==0){ // global
                		genGlobalVar(tempType,$1);
                	}
                }
                  ASSIGN_OP logical_expression
				{
					struct param_sem *ptr;					
					ptr = createParam( createIdList( $1 ), createPType( VOID_t ) );
					$$ = createVarDeclParam( ptr, $4 );		
					$$->isInit = __TRUE;


                	if(scope!=0){ //local
                		genAssignValue("local",tempLocal,NULL,tempType);
						genI2F2 = 0;
                	}	
					tempLocal++;
				}
                | ID 
				{
					struct param_sem *ptr;					
					ptr = createParam( createIdList( $1 ), createPType( VOID_t ) );
					$$ = createVarDeclParam( ptr, 0 );

					if(scope==0){ // global
                		genGlobalVar(tempType,$1);
                	}	
                	tempLocal++;			
				}
                ;
		 
initial_array : L_BRACE literal_list R_BRACE { $$ = $2; }
			  ;

literal_list : literal_list COMMA logical_expression
				{
					struct expr_sem *ptr;
					for( ptr=$1; (ptr->next)!=0; ptr=(ptr->next) );				
					ptr->next = $3;
					$$ = $1;
				}
             | logical_expression
				{
					$$ = $1;
				}
             |
             ;

const_decl 	: CONST 
			{
				isConst = 1;
			}
			  scalar_type const_list SEMICOLON
			{
				struct SymNode *newNode;				
				struct constParam *ptr;
				for( ptr=$4; ptr!=0; ptr=(ptr->next) ){
					if( verifyRedeclaration( symbolTable, ptr->name, scope ) == __TRUE ){//no redeclare
						if( ptr->value->category != $3->type ){//type different
							if( !(($3->type==FLOAT_t || $3->type == DOUBLE_t ) && ptr->value->category==INTEGER_t) ) {
								if(!($3->type==DOUBLE_t && ptr->value->category==FLOAT_t)){	
									fprintf( stdout, "########## Error at Line#%d: const type different!! ##########\n", linenum );
									semError = __TRUE;	
								}
								else{
									newNode = createConstNode( ptr->name, scope, $3, ptr->value );
									
									/* gen var */
									if(scope==0){ //global
										newNode->isLocal = 0;
										genGlobalVar(ptr->value->category,ptr->name);
									}
									else{ //local
										newNode->isLocal = 1;
										newNode->stackNum = counterNum;
										counterNum++;
									}
									genI2F2 = 0;

									insertTab( symbolTable, newNode );
								}
							}							
							else{
								newNode = createConstNode( ptr->name, scope, $3, ptr->value );
								insertTab( symbolTable, newNode );
							}
						}
						else{
							newNode = createConstNode( ptr->name, scope, $3, ptr->value );
							insertTab( symbolTable, newNode );
						}
					}
				}

				isConst = 0;
			}
			;

const_list : const_list COMMA ID ASSIGN_OP literal_const
			{				
				addConstParam( $1, createConstParam( $5, $3 ) );
				$$ = $1;
			}
		   | ID ASSIGN_OP literal_const
			{
				$$ = createConstParam( $3, $1 );	
			}
		   ;

array_decl : ID dim 
			{
				$$ = createParam( createIdList( $1 ), $2 );
			}
		   ;

dim : dim ML_BRACE INT_CONST MR_BRACE
		{
			if( $3 == 0 ){
				fprintf( stdout, "########## Error at Line#%d: array size error!! ##########\n", linenum );
				semError = __TRUE;
			}
			else
				increaseArrayDim( $1, 0, $3 );			
		}
	| ML_BRACE INT_CONST MR_BRACE	
		{
			if( $2 == 0 ){
				fprintf( stdout, "########## Error at Line#%d: array size error!! ##########\n", linenum );
				semError = __TRUE;
			}			
			else{		
				$$ = createPType( VOID_t ); 			
				increaseArrayDim( $$, 0, $2 );
			}		
		}
	;
	
compound_statement : {scope++;}L_BRACE var_const_stmt_list R_BRACE
					{ 
						// print contents of current scope
						if( Opt_Symbol == 1 )
							printSymTable( symbolTable, scope );
							
						deleteScope( symbolTable, scope );	// leave this scope, delete...
						scope--; 
					}
				   ;

var_const_stmt_list : var_const_stmt_list statement	
				    | var_const_stmt_list var_decl
					| var_const_stmt_list const_decl
				    |
				    ;

statement : compound_statement
		  | simple_statement
		  | conditional_statement
		  | while_statement
		  | for_statement
		  | function_invoke_statement
		  | jump_statement
		  ;		

simple_statement : variable_reference
					{	
						struct SymNode *nodeType = lookupSymbol(symbolTable,$1->varRef->id,scope,__FALSE);
						if((nodeType->type->type==FLOAT_t)||(nodeType->type->type==DOUBLE_t)){
							genI2F = 1;
						}
					}
				   ASSIGN_OP logical_expression SEMICOLON
					{
						// check if LHS exists
						__BOOLEAN flagLHS = verifyExistence( symbolTable, $1, scope, __TRUE );
						// id RHS is not dereferenced, check and deference
						__BOOLEAN flagRHS = __TRUE;
						if( $4->isDeref == __FALSE ) {
							flagRHS = verifyExistence( symbolTable, $4, scope, __FALSE );
						}
						// if both LHS and RHS are exists, verify their type
						if( flagLHS==__TRUE && flagRHS==__TRUE ){
							verifyAssignmentTypeMatch( $1, $4 );

							/* gen assign */
							struct SymNode *L_node = lookupSymbol(symbolTable,$1->varRef->id,scope,__FALSE);
							char *varKind;

							if(L_node->isLocal==0){ // global
								varKind = (char*)malloc(sizeof(char)*6);
								strncpy(varKind,"global",6);
							}
							else{ // local
								varKind = (char*)malloc(sizeof(char)*5);
								strncpy(varKind,"local",5);
							}
							
							if(L_node->category==PARAMETER_t){
								genAssignValue(varKind,localNo,L_node->name,L_node->type->type);											
							}
							else{
								genAssignValue(varKind,L_node->stackNum,L_node->name,L_node->type->type);	
							}
						}	

						genI2F = 0;	
						genI2F2 = 0;				
					}
				 | PRINT {genPrintStart();}
				   logical_expression SEMICOLON 
				   { 
				   	verifyScalarExpr( $3, "print" ); 
				   	genPrintEnd($3);
				   	genI2F2 = 0;
				   }
				 | READ variable_reference SEMICOLON 
					{  
						if( verifyExistence( symbolTable, $2, scope, __TRUE ) == __TRUE ){
							verifyScalarExpr( $2, "read" ); 
							
							/* gen value */
							struct expr_sem *findName = $2;
							struct SymNode *node = lookupSymbol(symbolTable,$2->varRef->id,scope,__FALSE);
							
							if(node->category==PARAMETER_t){
								genRead(findName,node->isLocal,localNo);
							}
							else{
								genRead(findName,node->isLocal,node->stackNum);
							}
						}						
							
					}
				 ;

conditional_statement : if_statement
						{				
							fprintf(fout_code,"Lfinish_%d:\n",ifLevelCur);				  		  		
							ifLevelCur--;
					  		ifLevelStore--;
						}						
					  | if_statement 
					  	ELSE compound_statement
					  	{	
					  		fprintf(fout_code,"Lfinish_%d:\n",ifLevelCur);				  		
					  		ifLevelCur--;
					  		ifLevelStore--;
					  	}
					  ;
if_statement : 	IF 
				{ 	
					if(ifLevelStore==0){
						ifLevelCur = ifLevelNext;
					}
					else{
						ifLevelCur++;
					}
					ifLevelStore++;
					ifLevelNext++;
				}
				L_PAREN conditional_if  R_PAREN
				{
					fprintf(fout_code,"\ticonst_0\n");
					fprintf(fout_code,"\tisub\n");
					fprintf(fout_code,"\tifeq Lexit_%d\n",ifLevelCur);
				} 
				compound_statement
				{
					fprintf(fout_code,"\tgoto Lfinish_%d\n",ifLevelCur);
					fprintf(fout_code,"Lexit_%d:\n",ifLevelCur);
				}
			  ;					 

conditional_if : logical_expression { verifyBooleanExpr( $1, "if" ); };;					  

				
while_statement : WHILE 
				{
					if(loopLevelStore==0){
						loopLevelCur = loopLevelNext;
					}
					else{
						loopLevelCur++;
					}
					loopLevelStore++;
					loopLevelNext++;
					fprintf(fout_code,"whileLabel_%d:\n",loopLevelCur); 					
				}
				  L_PAREN logical_expression 
				{ 
					verifyBooleanExpr( $4, "while" ); 
					fprintf(fout_code,"\ticonst_0\n");
					fprintf(fout_code,"\tisub\n");
					fprintf(fout_code,"\tifeq Wexit_%d\n",loopLevelCur);
				} 
				R_PAREN 
				{ 
					inloop++; 
				}
				compound_statement 
				{ 
					inloop--;
					fprintf(fout_code,"\tgoto whileLabel_%d\n",loopLevelCur); 
					fprintf(fout_code,"Wexit_%d:\n",loopLevelCur);
					loopLevelCur--;
					loopLevelStore--;
				}
				| 
				{ 
					inloop++; 
					if(loopLevelStore==0){
						loopLevelCur = loopLevelNext;
					}
					else{
						loopLevelCur++;
					}
					loopLevelStore++;
					loopLevelNext++;
					fprintf(fout_code,"whileLabel_%d:\n",loopLevelCur);
				} 
				DO compound_statement 
				WHILE L_PAREN logical_expression R_PAREN SEMICOLON  
					{ 
						 verifyBooleanExpr( $6, "while" );
						 inloop--; 

						 fprintf(fout_code,"\ticonst_1\n");
						 fprintf(fout_code,"\tisub\n");
						 fprintf(fout_code,"\tifeq whileLabel_%d\n",loopLevelCur);
						 loopLevelCur--;
						 loopLevelStore--;
					}
				;


				
for_statement : FOR L_PAREN	initial_expression SEMICOLON
				{
					if(loopLevelStore==0){
						loopLevelCur = loopLevelNext;
					}
					else{
						loopLevelCur++;
					}
					loopLevelStore++;
					loopLevelNext++;
					fprintf(fout_code,"whileLabel_%d:\n",loopLevelCur);
				}  
				control_expression SEMICOLON
				{
					fprintf(fout_code,"\ticonst_1\n");
					fprintf(fout_code,"\tisub\n");
					fprintf(fout_code,"\tifeq whileComLabel_%d\n",loopLevelCur);
					fprintf(fout_code,"\tgoto Wexit_%d\n",loopLevelCur);
					fprintf(fout_code,"whileInLabel_%d:\n",loopLevelCur);
				} 
				increment_expression R_PAREN  
				{ 
					inloop++;
					fprintf(fout_code,"\tgoto whileLabel_%d\n",loopLevelCur); 
					fprintf(fout_code,"whileComLabel_%d:\n",loopLevelCur);

				}
				compound_statement  
				{ 
					inloop--; 
					fprintf(fout_code,"\tgoto whileInLabel_%d\n",loopLevelCur); 
					fprintf(fout_code,"Wexit_%d:\n",loopLevelCur);
					loopLevelCur--;
					loopLevelStore--;
				}
			  ;

initial_expression : initial_expression COMMA statement_for		
				   | initial_expression COMMA logical_expression
				   | logical_expression	
				   | statement_for
				   |
				   ;

control_expression : control_expression COMMA statement_for
				   {
						fprintf( stdout, "########## Error at Line#%d: control_expression is not boolean type ##########\n", linenum );
						semError = __TRUE;	
				   }
				   | control_expression COMMA logical_expression
				   {
						if( $3->pType->type != BOOLEAN_t ){
							fprintf( stdout, "########## Error at Line#%d: control_expression is not boolean type ##########\n", linenum );
							semError = __TRUE;	
						}
				   }
				   | logical_expression 
					{ 
						if( $1->pType->type != BOOLEAN_t ){
							fprintf( stdout, "########## Error at Line#%d: control_expression is not boolean type ##########\n", linenum );
							semError = __TRUE;	
						}
					}
				   | statement_for
				   {
						fprintf( stdout, "########## Error at Line#%d: control_expression is not boolean type ##########\n", linenum );
						semError = __TRUE;	
				   }
				   |
				   ;

increment_expression : increment_expression COMMA statement_for
					 | increment_expression COMMA logical_expression
					 | logical_expression
					 | statement_for
					 |
					 ;

statement_for 	: variable_reference ASSIGN_OP logical_expression
					{
						// check if LHS exists
						__BOOLEAN flagLHS = verifyExistence( symbolTable, $1, scope, __TRUE );
						// id RHS is not dereferenced, check and deference
						__BOOLEAN flagRHS = __TRUE;
						if( $3->isDeref == __FALSE ) {
							flagRHS = verifyExistence( symbolTable, $3, scope, __FALSE );
						}
						// if both LHS and RHS are exists, verify their type
						if( flagLHS==__TRUE && flagRHS==__TRUE ){
							verifyAssignmentTypeMatch( $1, $3 );

							/* gen assign */
							struct SymNode *L_node = lookupSymbol(symbolTable,$1->varRef->id,scope,__FALSE);
							char *varKind;

							if(L_node->scope==0){ // global
								varKind = (char*)malloc(sizeof(char)*6);
								strncpy(varKind,"global",6);
							}
							else{ // local
								varKind = (char*)malloc(sizeof(char)*5);
								strncpy(varKind,"local",5);
							}
							
							
							if(L_node->category==PARAMETER_t){
								genAssignValue(varKind,localNo,L_node->name,L_node->type->type);	
							}
							else{
								genAssignValue(varKind,L_node->stackNum,L_node->name,L_node->type->type);	
							}
						}	

						genI2F = 0;	
						genI2F2 = 0;						
					}
					;
					 
					 
function_invoke_statement : ID L_PAREN logical_expression_list R_PAREN SEMICOLON
							{
								verifyFuncInvoke( $1, $3, symbolTable, scope );
								genFunctInvoke($1,symbolTable,scope);
							}
						  | ID L_PAREN R_PAREN SEMICOLON
							{
								verifyFuncInvoke( $1, 0, symbolTable, scope );
								genFunctInvoke($1,symbolTable,scope);
							}
						  ;

jump_statement : CONTINUE SEMICOLON
				{
					if( inloop <= 0){
						fprintf( stdout, "########## Error at Line#%d: continue can't appear outside of loop ##########\n", linenum ); semError = __TRUE;
					}
					else{
						fprintf(fout_code,"\tgoto whileLabel_%d\n",loopLevelCur); 
					}
				}
			   | BREAK SEMICOLON 
				{
					if( inloop <= 0){
						fprintf( stdout, "########## Error at Line#%d: break can't appear outside of loop ##########\n", linenum ); semError = __TRUE;
					}
					else{
						fprintf(fout_code,"\tgoto Wexit_%d\n",loopLevelCur); 
					}
				}
			   | RETURN logical_expression SEMICOLON
				{
					verifyReturnStatement( $2, funcReturn );

					/* gen ireturn */
					if(isMain!=1){
						genFunctEnd(0);
					}
					
				}
			   ;

variable_reference : ID
					{
						$$ = createExprSem( $1 );
					}
				   | variable_reference dimension
					{	
						increaseDim( $1, $2 );
						$$ = $1;
					}
				   ;

dimension : ML_BRACE arithmetic_expression MR_BRACE
			{
				$$ = verifyArrayIndex( $2 );
			}
		  ;
		  
logical_expression : logical_expression OR_OP logical_term
					{
						verifyAndOrOp( $1, OR_t, $3 );
						$$ = $1;
						genI2F2 = 0;
					}
				   | logical_term 
				   { 
				   		$$ = $1; 
				   		genI2F2 = 0;
				   }
				   ;

logical_term : logical_term AND_OP logical_factor
				{
					verifyAndOrOp( $1, AND_t, $3 );
					$$ = $1;
				}
			 | logical_factor { $$ = $1; }
			 ;

logical_factor : NOT_OP logical_factor
				{
					verifyUnaryNOT( $2 );
					$$ = $2;
				}
			   | relation_expression { $$ = $1; }
			   ;

relation_expression : arithmetic_expression relation_operator arithmetic_expression
					{
						verifyRelOp( $1, $2, $3 );

						int isInt = 1;
						SEMTYPE type1 = $1->pType->type;
						SEMTYPE type2 = $3->pType->type;
						if((type1==FLOAT_t)||(type2==FLOAT_t)){
							isInt = 0;
						}
						if((type1==DOUBLE_t)||(type2==DOUBLE_t)){
							isInt = 0;
						}
						genBooleanLabel(trueLevel,$2,isInt);
						trueLevel++;
						
						$$ = $1;
					}
					| arithmetic_expression { $$ = $1; }
					;

relation_operator : LT_OP { $$ = LT_t; }
				  | LE_OP { $$ = LE_t; }
				  | EQ_OP { $$ = EQ_t; }
				  | GE_OP { $$ = GE_t; }
				  | GT_OP { $$ = GT_t; }
				  | NE_OP { $$ = NE_t; }
				  ;

arithmetic_expression : arithmetic_expression add_op term
			{
				verifyArithmeticOp( $1, $2, $3 );
				$$ = $1;
			}
                   | relation_expression { $$ = $1; }
		   | term { $$ = $1; }
		   ;

add_op	: ADD_OP { $$ = ADD_t; }
		| SUB_OP { $$ = SUB_t; }
		;
		   
term : term mul_op factor
		{
			if( $2 == MOD_t ) {
				verifyModOp( $1, $3 );
			}
			else {
				verifyArithmeticOp( $1, $2, $3 );
			}
			$$ = $1;
		}
     | factor { $$ = $1; }
	 ;

mul_op 	: MUL_OP { $$ = MUL_t; }
		| DIV_OP { $$ = DIV_t; }
		| MOD_OP { $$ = MOD_t; }
		;
		
factor : variable_reference
		{
			verifyExistence( symbolTable, $1, scope, __FALSE );
			$$ = $1;
			$$->beginningOp = NONE_t;

			/* gen value */
			struct expr_sem *findName = $1;
			struct SymNode *node = lookupSymbol(symbolTable,$1->varRef->id,scope,__FALSE);
			char *varKind;

			if(node->category==CONSTANT_t){ // const
				switch(node->type->type){
					case INTEGER_t:
						genGetValue("int",node->attribute->constVal->value.integerVal,0,0,NULL,INTEGER_t);
						break;
					case FLOAT_t:
						genGetValue("float",0,node->attribute->constVal->value.floatVal,0,NULL,FLOAT_t);
						break;
					case DOUBLE_t:
						genGetValue("double",0,0,(float)node->attribute->constVal->value.doubleVal,NULL,FLOAT_t);
						break;
					case BOOLEAN_t:
						if(node->attribute->constVal->value.booleanVal==__TRUE){ // true
							genGetValue("bool",0,0,0,"true",BOOLEAN_t);
						}
						else{ // false
							genGetValue("bool",0,0,0,"false",BOOLEAN_t);
						}
						break;
					case STRING_t:
						genGetValue("string",0,0,0,node->attribute->constVal->value.stringVal,STRING_t);
						break;
				}			
			}
			else{

				if(node->scope==0){ // global
					varKind = (char*)malloc(sizeof(char)*6);
					strncpy(varKind,"global",6);
				}
				else{ // local
					varKind = (char*)malloc(sizeof(char)*5);
					strncpy(varKind,"local",5);
				}

				if(node->category==PARAMETER_t){
					genGetValue(varKind,localNo,0,0,$1->varRef->id,node->type->type);
				}
				else{
					genGetValue(varKind,node->stackNum,0,0,$1->varRef->id,node->type->type);
				}
			}
		}
	   | SUB_OP variable_reference
		{
			if( verifyExistence( symbolTable, $2, scope, __FALSE ) == __TRUE )
			verifyUnaryMinus( $2 );
			$$ = $2;
			$$->beginningOp = SUB_t;

			/* gen value */
			struct expr_sem *findName = $2;
			struct SymNode *node = lookupSymbol(symbolTable,$2->varRef->id,scope,__FALSE);
			char *varKind;

			if(node->isLocal==0){ // global
				varKind = (char*)malloc(sizeof(char)*6);
				strncpy(varKind,"global",6);
			}
			else{ // local
				varKind = (char*)malloc(sizeof(char)*5);
				strncpy(varKind,"local",5);
			}

			if(node->category==PARAMETER_t){
				genGetValue(varKind,localNo,0,0,$2->varRef->id,node->type->type);
			}
			else{
				genGetValue(varKind,node->stackNum,0,0,$2->varRef->id,node->type->type);
			}

			/* gen sub */
			if(node->type->type==INTEGER_t){ // int
				fprintf(fout_code,"\tineg\n");
			}
			else if((node->type->type==FLOAT_t)||(node->type->type==DOUBLE_t)){ // float
				fprintf(fout_code,"\tfneg\n");
			}
		}		
	   | L_PAREN logical_expression R_PAREN
		{
			$2->beginningOp = NONE_t;
			$$ = $2; 
		}
	   | SUB_OP L_PAREN logical_expression R_PAREN
		{
			verifyUnaryMinus( $3 );
			$$ = $3;
			$$->beginningOp = SUB_t;

			struct expr_sem *nodeEx = $3;			
			/* gen sub */
			if(nodeEx->pType->type==INTEGER_t){ // int
				fprintf(fout_code,"\tineg\n");
			}
			else if((nodeEx->pType->type==FLOAT_t)||(nodeEx->pType->type==DOUBLE_t)){ // float
				fprintf(fout_code,"\tfneg\n");
			}			
		}
	   | ID L_PAREN logical_expression_list R_PAREN
		{
			$$ = verifyFuncInvoke( $1, $3, symbolTable, scope );
			$$->beginningOp = NONE_t;

			/* gen funct invoke */
			genFunctInvoke($1,symbolTable,scope);
		}
	   | SUB_OP ID L_PAREN logical_expression_list R_PAREN
	    {
			$$ = verifyFuncInvoke( $2, $4, symbolTable, scope );
			$$->beginningOp = SUB_t;

			/* gen funct invoke */
			genFunctInvoke($2,symbolTable,scope);

			struct SymNode *node = lookupSymbol(symbolTable,$2,scope,__FALSE);			
			/* gen sub */
			if(node->type->type==INTEGER_t){ // int
				fprintf(fout_code,"\tineg\n");
			}
			else if((node->type->type==FLOAT_t)||(node->type->type==DOUBLE_t)){ // float
				fprintf(fout_code,"\tfneg\n");
			}
		}
	   | ID L_PAREN R_PAREN
		{
			$$ = verifyFuncInvoke( $1, 0, symbolTable, scope );
			$$->beginningOp = NONE_t;

			/* gen funct invoke */
			genFunctInvoke($1,symbolTable,scope);
		}
	   | SUB_OP ID L_PAREN R_PAREN
		{
			$$ = verifyFuncInvoke( $2, 0, symbolTable, scope );
			$$->beginningOp = SUB_OP;

			/* gen funct invoke */
			genFunctInvoke($2,symbolTable,scope);

			struct SymNode *node = lookupSymbol(symbolTable,$2,scope,__FALSE);			
			/* gen sub */
			if(node->type->type==INTEGER_t){ // int
				fprintf(fout_code,"\tineg\n");
			}
			else if((node->type->type==FLOAT_t)||(node->type->type==DOUBLE_t)){ // float
				fprintf(fout_code,"\tfneg\n");
			}
		}
	   | literal_const
	    {
			  $$ = (struct expr_sem *)malloc(sizeof(struct expr_sem));
			  $$->isDeref = __TRUE;
			  $$->varRef = 0;
			  $$->pType = createPType( $1->category );
			  $$->next = 0;
			  if( $1->hasMinus == __TRUE ) {
			  	$$->beginningOp = SUB_t;
			  }
			  else {
				$$->beginningOp = NONE_t;
			  }
		}
	   ;

logical_expression_list : logical_expression_list COMMA logical_expression
						{
			  				struct expr_sem *exprPtr;
			  				for( exprPtr=$1 ; (exprPtr->next)!=0 ; exprPtr=(exprPtr->next) );
			  				exprPtr->next = $3;
			  				$$ = $1;
						}
						| logical_expression { $$ = $1; }
						;

scalar_type : INT 
			{ 
				$$ = createPType( INTEGER_t );

		  		tempType = INTEGER_t;
		   		tempLocal = counterNum; 
			}
			| DOUBLE 
			{ 
				$$ = createPType( DOUBLE_t );

				tempType = DOUBLE_t;
		   		tempLocal = counterNum;  
			}
			| STRING 
			{ 
				$$ = createPType( STRING_t ); 

				tempType = STRING_t;
		   		tempLocal = counterNum; 
			}
			| BOOL 
			{ 
				$$ = createPType( BOOLEAN_t ); 

				tempType = BOOLEAN_t;
		   		tempLocal = counterNum; 
			}
			| FLOAT 
			{ 
				$$ = createPType( FLOAT_t );

				tempType = FLOAT_t;
		   		tempLocal = counterNum; 
		   	}
			;
 
literal_const : INT_CONST
				{
					int tmp = $1;
					$$ = createConstAttr( INTEGER_t, &tmp );

					if(isConst!=1){
						if(genI2F==0){
							genI2F2 = 1;
						}
						genGetValue("int",tmp,0,0,NULL,ERROR_t);
					}
				}
			  | SUB_OP INT_CONST
				{
					int tmp = -$2;
					$$ = createConstAttr( INTEGER_t, &tmp );

					if(isConst!=1){
						if(genI2F==0){
							genI2F2 = 1;
						}
						tmp = $2;
						genGetValue("int",tmp,0,0,NULL,ERROR_t);
						fprintf(fout_code,"\tineg\n");
					}
				}
			  | FLOAT_CONST
				{
					float tmp = $1;
					$$ = createConstAttr( FLOAT_t, &tmp );

					if(isConst!=1){
						genGetValue("float",0,tmp,0,NULL,ERROR_t);
					}
				}
			  | SUB_OP FLOAT_CONST
			    {
					float tmp = -$2;
					$$ = createConstAttr( FLOAT_t, &tmp );

					if(isConst!=1){
						tmp = $2;
						genGetValue("float",0,tmp,0,NULL,ERROR_t);
						fprintf(fout_code,"\tfneg\n");
					}
				}
			  | SCIENTIFIC
				{
					double tmp = $1;
					$$ = createConstAttr( DOUBLE_t, &tmp );

					if(isConst!=1){
						genGetValue("double",0,0,(float)tmp,NULL,ERROR_t);
					}
				}
			  | SUB_OP SCIENTIFIC
				{
					double tmp = -$2;
					$$ = createConstAttr( DOUBLE_t, &tmp );

					if(isConst!=1){
						tmp = $2;
						genGetValue("double",0,0,(float)tmp,NULL,ERROR_t);
						fprintf(fout_code,"\tfneg\n");
					}
				}
			  | STR_CONST
				{
					$$ = createConstAttr( STRING_t, $1 );

					if(isConst!=1){
						genGetValue("string",0,0,0,(char *)$1,ERROR_t);
					}
				}
			  | TRUE
				{
					SEMTYPE tmp = __TRUE;
					$$ = createConstAttr( BOOLEAN_t, &tmp );

					if(isConst!=1){
						genGetValue("bool",0,0,0,"true",ERROR_t);
					}
				}
			  | FALSE
				{
					SEMTYPE tmp = __FALSE;
					$$ = createConstAttr( BOOLEAN_t, &tmp );

					if(isConst!=1){
						genGetValue("bool",0,0,0,"false",ERROR_t);
					}
				}
			  ;
%%

int yyerror( char *msg )
{
    fprintf( stderr, "\n|--------------------------------------------------------------------------\n" );
	fprintf( stderr, "| Error found in Line #%d: %s\n", linenum, buf );
	fprintf( stderr, "|\n" );
	fprintf( stderr, "| Unmatched token: %s\n", yytext );
	fprintf( stderr, "|--------------------------------------------------------------------------\n" );
	exit(-1);
}


