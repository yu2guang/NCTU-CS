	...
	if( (op1->pType->type)==ERROR_t || (op2->pType->type)==ERROR_t ) {
		...
		if( op1->pType->type==STRING_t && op2->pType->type==STRING_t ) {	// string concatenation			
			...
			switch( operator ) {
	 			case ADD_t:
	 				if(isFloat==1){
	 					fprintf(fout_code,"\tfadd\n");
	 				}
	 				else{
						fprintf(fout_code,"\tiadd\n");
					}
					break;
	 			case SUB_t:
					if(isFloat==1){
	 					fprintf(fout_code,"\tfsub\n");
	 				}
	 				else{
	 			...
	 			case MUL_t:
					if(isFloat==1){
	 					fprintf(fout_code,"\tfmul\n");
	 				}
	 				else{
						fprintf(fout_code,"\timul\n");
					}
					break;
	 			case DIV_t:
					if(isFloat==1){
	 					fprintf(fout_code,"\tfdiv\n");
	 				}
	 				else{
						fprintf(fout_code,"\tidiv\n");
					}
					break;
			}
		}
...
void verifyAndOrOp( struct expr_sem *op1, OPERATOR operator, struct expr_sem *op2 )
{
	if( (op1->pType->type)==ERROR_t || (op2->pType->type)==ERROR_t ) {
		...
		switch( operator ) {
	 		case AND_t:
				fprintf(fout_code,"\tiand\n");
				break;
	 		case OR_t:
	 			fprintf(fout_code,"\tior\n");
				break;
		}
	}
}
