...
int cur_pos;
int mode = 0; // 0:group #; 1:temperatures
OneWire_t* pinyugly = NULL;
...
void SysTick_Handler(void){
	...
	if(chan_mode==0){
		...
		if(mode==0){
			Init_SysTick(400000*3); //0.3 sec
		}
		else if(mode==1){
			Init_SysTick(4000000); //1 sec
		}
	...
	if(mode==0){
		...
		float temperature;
		DS18B20_ConvT(pinyugly,TM_DS18B20_Resolution_11bits);
		DS18B20_Read(pinyugly,&temperature);

		//temperature = &a;

		int tem_int = temperature;
		char send;
		int position = 0x80;
		send = tem_int/10 + '0';
		WriteToLCD(position, 0);
		WriteToLCD(send, 1);
		position++;
		send = tem_int%10 + '0';
		WriteToLCD(position, 0);
		...
		float tem_float = temperature;
		int one_digit;
		tem_float -= tem_int;
		for(int i=1; i<=13; i++){
			one_digit = tem_float*10;
			send = one_digit + '0';
			position++;
			WriteToLCD(position, 0);
			WriteToLCD(send,1);
			tem_float = tem_float*10 - one_digit;
		}
...
int main(){
	...
	DS18B20_SetResolution(pinyugly,TM_DS18B20_Resolution_11bits);

	Init_SysTick(400000*3);
