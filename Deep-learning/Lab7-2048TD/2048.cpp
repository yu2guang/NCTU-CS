...
public:
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for (path.pop_back() /* terminal state */; path.size(); path.pop_back()) {
			state& move = path.back();			
			// error = estimate(next_state) - (estimate(move.before_state()) - move.reward());			
			// update(move.before_state(), alpha * error);			
			// next_state = move.before_state();
...
int main(int argc, const char* argv[]) {
	...
	// sprintf(filename, "./result/7/weight2048_7_1_64");
	// tdl.load(filename);

	// open file to store score
	sprintf(filename, "./result/%d/score%d_%d.txt", version, version, piece);
	...
	for (size_t n = 1; n <= total; n++) {
		...
		float tmp = tdl.make_statistic(n, b, score);
		if((tmp>por)&&(n%1000==0)){
			sprintf(filename, "./result/%d/weight2048_%d_%d_%d", version, version, piece, int(tmp));
			tdl.save(filename);
			por = tmp;
		}
	...
	// sprintf(filename, "./result/%d/weight2048%d_%d", version, version, piece);
	// tdl.save(filename);
	sprintf(filename, "./result/%d/weight2048_%d_%d_0", version, version, piece);
	tdl.save(filename);
